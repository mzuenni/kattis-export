...
public class PlugItInChris_PushRelabelRebase {
	...
	public static void main(String[] args) {
		...
		long[][] flow = new long[graphsize][graphsize];
		pushRelabel(cap, flow, source, target);
		
		long max = 0;
		for (int i = 2; i < m + 2; i++) {
			cap[source][i] = 3;
			long[][] flowcop = new long[graphsize][graphsize];
			for (int j = 0; j < graphsize; j++) {
				flowcop[j] = Arrays.copyOf(flow[j], graphsize);
			}
			
			long result = pushRelabel(cap, flowcop, source, target);
			cap[source][i] = 1;
			if (result > max) {
				max = result;
			}
