...
int calcMaxFlow(int src, int sink, int n, int iniF) {
	int ret = iniF;
	if (!iniF)
		FOR(i,0,n)
			FOR(j,0,sz(e[i]))
...
void safeFlow(int n){
	FOR(i,0,n) FORIT(j,e[i]) j->sflow = j->flow;
}

void restoreFlow(int n){
	FOR(i,0,n) FORIT(j,e[i]) j->flow = j->sflow;
}
...
int main(){
	...
	// baseline 
	int base = calcMaxFlow(0,1,2+M+N,0);
	safeFlow(2+M+N);
	...
	FOR(i,0,M){
		addEdge(0,2+i,2,0);
		restoreFlow(2+M+N);
		int cur = calcMaxFlow(0,1,2+M+N,base);
		if (cur > mm) mm = cur;
		e[0].pop_back();
		e[2+i].pop_back();
