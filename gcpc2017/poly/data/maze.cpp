...
void dfs(int x, int y){
		...
		while (done != 0x0F){
				int nei = rand() % 4;
				if (done & (1 << nei)) continue;
				done |= 1 << nei;
				int nx = x + dx[nei];
				int ny = y + dy[nei];
				if (nx < 0 || nx >= N) continue;
				if (ny < 0 || ny >= N) continue;
				if (visi[nx][ny]) continue;

				go[x][y][nei] = true;
				//cerr << "GO " << x << " " << y << " " << nei << endl;
				dfs(nx,ny);
		}
}
...
void generate(int x, int y, int dir, int ori){
		call ++;	
		p.push_back(pt(10*x + smul*bx[corr(dir,ori)], 10*y + smul*by[corr(dir,ori)]));
		//cerr << "GEN " << x << " " << y << " " << corr(dir,ori) << endl;
		FOR(i,0,4) {
				int nei = (8 + /*ori*/i + dir) % 4;
				//cerr << "GEN " << x << " " << y << " " << corr(nei,ori) << " GO " << go[x][y][corr(nei,ori)] << endl;
				int nx = x + dx[corr(nei,ori)];
				int ny = y + dy[corr(nei,ori)];

				int doX = 1 - abs(dx[corr(nei,ori)]);
				int doY = 1 - abs(dy[corr(nei,ori)]);
				p.push_back(pt(10*x + smul*bx[corr(nei,ori)], 10*y + smul*by[corr(nei,ori)]));
				if (call != -1 && go[x][y][corr(nei,ori)] && (corr(nei,ori)+2)%4 != nei) generate (nx,ny,(corr(nei,ori)+2)%4, -1);
		}
}
...
int main(){
		...
		//cerr << "P " << sz(p) << endl;
		if (rot) FOR(i,0,sz(p)) p[i] = rotate(p[i],PI/4);
		cout << sz(p) << endl;
		FORIT(i,p) cout << int(i->x) << " " << int(i->y) << endl;
}
