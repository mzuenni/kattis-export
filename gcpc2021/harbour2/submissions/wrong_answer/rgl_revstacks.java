...
public class rgl_revstacks {
  private static long solve(int[] s1, int[] s2) {
    final int n = s1.length + s2.length;

    final Map<Integer, Integer> position = new HashMap<>();
    IntStream.range(0, s1.length).forEach(i -> position.put(s1[i], s1.length-1-i));
    IntStream.range(0, s2.length).forEach(i -> position.put(s2[i], s1.length-0+i));
