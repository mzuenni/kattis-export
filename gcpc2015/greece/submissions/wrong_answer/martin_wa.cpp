...
/** Greece 
		...
		Assuming only one edge for each pair of vertices -> WA.
...
void dij(int v)
{
	...
	while(!q.empty())
	{
		...
		for(int i=0;i<N;i++) 
		{
			if(B[u][i] >= INF) continue;
			if(B[v][u] + B[u][i] < B[v][i])
			{
				B[v][i] = B[v][u] + B[u][i];
				q.push(pair<int,int>(B[v][i], i));
			}
		}
	}
}
...
{  
		...
		for(int i=0;i<M;i++)
		{
			int s,d,t;
			cin>>s>>d>>t;
			B[s][d] = B[d][s] = t; // missed the double edge
		}
		for(int i=0;i<(int)cities.size();i++) dij(cities[i]);
