...
first used dijkstra for reducing the graph, then TSP in (n^2*2^n). Uses early exit for subsets 
with the wrong number of visited sites.

Still may be too slow...

Doesn't handle right the case, when only Athen is wanted: At the end of the TSP it assume not to ne in 0. 
*/

public class GreeceEgorOnlyAthenWrong {

	
	public static void main(String[] args) throws Exception{
        new GreeceEgorOnlyAthenWrong().run();
    }
	...
	int[] runTSP(int[][] dists, int taxiTime){
	   ...
	   for(int j=1;j<n;j++){//number of visited sides
		 ini3DArrayWithMaxInt(nextBest);  
		    for(int curState=(1<<j)-1;curState<MAX;curState++){
			if(Integer.bitCount(curState)!=j)
					continue;
	   ...
	   for(int taxi=0;taxi<2;taxi++)
		   for(int i=1;i<n;i++) //<--- Here we go: what if we still in Athen?
