...
int dp[2<<SMALLN][SMALLN];
int lo[2<<SMALLN][SMALLN];

int play(int n, int S, int s){
	if (S==(1<<s)){ lo[S][s] = sDist[s][0]; return sDist[s][0]; }
	int& v=dp[S][s];
	int& vl=lo[S][s];
	if (v>=0) return v;
	v=oo;
	...
	FOR(i,0,n) if (i!=s) if (S&(1<<i)) {
		int nd = sDist[s][i]+play(n,S-(1<<s),i);
		int nl = max(sDist[s][i], lo[S-(1<<s)][i]);
		if (nd < v) v = nd, vl = nl;
		else if (nd == v) vl = max(vl, nl);
	}
...
int main(){
		...
		int better = max(0,lo[(1<<nS)-1][0] - T);

		//cout << "R " << dp[(1<<nS)-1][0] << " " << dp[(1<<nS)-1][0] - better << endl; 
		
		if (dp[(1<<nS)-1][0] - better > G) cout << "impossible" << endl;
		else if (dp[(1<<nS)-1][0] <= G) cout << "possible without taxi" << endl;
