...
public class GreeceEgorSlow_n2dij {

	
	public static void main(String[] args) throws Exception{
        new GreeceEgorSlow_n2dij().run();
    }
	...
	int[] dijkstra(List<int[]>[] graph, int start){
		...
		while(true) {
			int n = -1;
			for (int i = 0; i < N; i++) if (!done[i] && (n == -1 || (dists[i] < dists[n]))) n = i;
			if (n == -1) break;
			if (dists[n] == Integer.MAX_VALUE) break;

			done[n] = true;
			for(int[] edge:graph[n]){
				int next=edge[1];
				int nextDist=dists[n]+edge[0];
