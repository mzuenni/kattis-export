...
int bt( int at, int collected, int budget ){
	if( budget < table[num_treasures][at] ) return 0;
	int best = 0;
	for( int i = 0; i < num_treasures; ++i )
		if( table[at][i] != std::numeric_limits<int>::max() && ( !(collected & (1<<i)) ) )
			best = max( best, bt( i, collected | (1<<i), budget - table[at][i] ) );		
	return 1 + best;
}
...
void calcTable(int src, int row ){
	...
	for( unsigned i = 0; i < num_treasures; ++i ) {
		if( distances[treasures[i]] != numeric_limits<int>::max() )
			table[row][i] = distances[treasures[i]];
	}
