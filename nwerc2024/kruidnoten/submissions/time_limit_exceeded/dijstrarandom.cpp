...
struct why {//no one would ever do this...
	ll key, dist, id;
	why(ll dist_, ll id_) : key(rng()), dist(dist_), id(id_) {}

	bool operator<(const why& o) const {
		return key < o.key;
	}
};
