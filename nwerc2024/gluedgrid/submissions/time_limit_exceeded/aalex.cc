...
bool isevenpermut(Iterator first, Iterator last) {
  ...
  for (Iterator it = first; it != last; ++it) {
    ...
    if (status[p / w][p % w] != CHECKING) return false;
    for (Iterator jt = it + 1; jt != last; ++jt) {
      auto [x2, y2] = *jt;
      if (x < x2 || (x == x2 && y < y2)) {
        if (p > puzzle[x2][y2] - 1) even = !even;
      } else {
        if (p < puzzle[x2][y2] - 1) even = !even;
      }
    }
  ...
  while (lena > 0) {
    ...
    if (progress[x][y] >= 0) {
      ...
      if (backreach[x2][y2] >= depth[x][y]) {
        if (!isevenpermut(stackb + bindex[x2][y2], stackb + lenb)) return false;
        lenb = bindex[x2][y2];
      }
    }
