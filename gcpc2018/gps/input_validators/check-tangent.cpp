...
int main() {
	...
	while (n--) {
		...
		double distance_to_tangent = earth_radius * abs(sat*me - 1);
		cout << "distance: " << distance_to_tangent << endl;
		if (distance_to_tangent < 0.0011) {
			cout << "too small distance: " << distance_to_tangent << endl;
			exit(43);
		}
	}
	exit(42);
