        ...
        for0(x, w) {
            for (ll dx = -1; dx <= 1; dx++) {
                for (ll dy = -1; dy <= 1; dy++) {
                    ...
                    if (mat[y + 0 * dy][x + 0 * dx] != 'k') continue;
                    if (mat[y + 1 * dy][x + 1 * dx] != 'i') continue;
                    if (mat[y + 2 * dy][x + 2 * dx] != 't') continue;
                    count++;
                }
            }
        }
...
int main() {
    ...
    if (oRow >= 1) {
        for (ll i = max(0ll, oCol - 2); i <= min(w, oCol + 2); i++) {
    ...
    string rest = string(c['k'], 'k') + string(c['t'], 't') + string(c['i'], 'i');
    vector<vector<char>> filled = fill(mat, rest);

    // if (!check(filled)) {
    //     rest = string(c['i'], 'i') + string(c['k'], 'k') + string(c['t'], 't');
    //     filled = fill(mat, rest);
    // }

    // if (!check(filled)) {
    //     rest = string(c['i'], 'i') + string(c['t'], 't') + string(c['k'], 'k');
    //     filled = fill(mat, rest);
    // }

    // if (!check(filled)) {
    //     rest = string(c['t'], 't') + string(c['k'], 'k') + string(c['i'], 'i');
    //     filled = fill(mat, rest);
    // }
