...
int main() {
	...
	for (int l = 0; l <= n; ++l) {
		...
		for (int r : down[l]) {
			...
			while (itf != pareto.end() && itf->second < newbest) {
				itf->second = newbest;
				++itf;
			}
			if (itb->first == r) {
		...
		if (l % 10000 == 0) {
			std::vector<std::pair<int, int>> npareto;
			npareto.push_back(pareto[0]);
			for (int i = 1; i < (int)(pareto.size()); ++i) {
				if (pareto[i].second > npareto.rbegin()->second) {
					npareto.push_back(pareto[i]);
				}
			}
			pareto = npareto;
		}
