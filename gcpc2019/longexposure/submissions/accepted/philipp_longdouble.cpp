...
const long double MY_PI = 3.14159265358979323846l;
...
int main() {
	...
	for(ll radius = 1, req = 4; (radius < MAXCIRCLE || painted) && it != covered.end(); radius++, req += 8) {
		...
		while(it != covered.end() && it->xx < r2) {
			long double area = MY_PI * (it->xx - oldr2);
			if(it->yy) {
				if(painted == req) {
					black += area;
					oldr2 = it->xx;
		...
		if(painted) {
			long double area = MY_PI * (r2 - oldr2);
			if(painted == req) {
				black += area;
			} else {
				grey += area;
	...
	cout.precision(15);
	cout << fixed << black << endl;
	cout << fixed << grey << endl;
}
