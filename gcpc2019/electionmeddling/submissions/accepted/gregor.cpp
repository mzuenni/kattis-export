...
int main(){
	...
	FOR(i,0,w){
		...
		while (v[i][0] + bribe <= valeq){
			...
			int currentVotes = v[i][0] + bribe;
			int nearlyBribed = (valeq - currentVotes) / (numeq + 1);
			int suffBribe =  nearlyBribed * numeq;

			int diff = valeq - nearlyBribed - (currentVotes  + suffBribe);
			if (diff >= 0) suffBribe += diff + (diff < numeq);
