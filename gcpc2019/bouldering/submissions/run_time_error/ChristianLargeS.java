...
// Solution for Bouldering that uses the actually given s
// Depending on heap space either produces a heap space error or a timelimit
public class ChristianLargeS {
    ...
    public static void main(String[] args) {
        ...
        while(!pq.isEmpty()) {
            Dist dist = pq.poll();
            if (dist.node == finindex) {
                break;
            }
            if (visited[dist.node][dist.stam]) {
                continue;
            }
            // never consider anything with higher stamina
            // visited[dist.node][dist.stam] = true;
            for (int i = dist.stam; i < s; i++) {
                visited[dist.node][i] = true;
            }
