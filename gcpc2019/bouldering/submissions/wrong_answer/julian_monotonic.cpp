...
vector<vector<double>> dijkstra(vector<vector<pair<int, double>>>& adj, int start) {
    ...
    priority_queue<tuple<double, int, int>> q; // distance, node id, stamina
    q.emplace(0, start, s);
    while (q.size()) {
        double d;
        int v, current_s;
        tie(d, v, current_s) = q.top();
        q.pop();
        if (dist[v][current_s] <= -d) continue;
        if (min_stamina[v] > current_s) continue;
        min_stamina[v] = current_s + 1;
        dist[v][current_s] = -d;
        for (auto p: adj[v]) {
            int new_s = current_s - stamina[p.first];
            if (new_s >= min_stamina[p.first]) q.emplace(d - p.second, p.first, new_s);
        }
    }
    return dist;
}
...
int main()
{
    ...
    for (size_t i = 0; i < pos.size(); ++i) {
        for (size_t j = i + 1; j < pos.size(); ++j) {
            ...
            if (d <= r) {
                if (pos[i].first >= pos[j].first) adj[i].push_back({j, d});
                if (pos[j].first >= pos[i].first) adj[j].push_back({i, d});
            }
        }
    }
