...
int main() {
    ...
    if (abs(mid1 - mid2) < EPS) {
        cout << "impossible\n";
        return 0;
    }
    pt mid = (mid1 + mid2) / (ld)2;
    pt dir = mid - mid1;
    if (abs(dir) < 1) dir = dir / abs(dir);
    dir = {dir.imag(), -dir.real()};
    dir += mid;

    for (auto& e : a1) {
        if (cross(mid, dir, e) > 0) {
            cout << "impossible\n";
            return 0;
        }
    ...
    sort(all(a1), [&](auto& x, auto& y) { return pair{round(x.real()), round(x.imag())} < pair{round(y.real()), round(y.imag())}; });
