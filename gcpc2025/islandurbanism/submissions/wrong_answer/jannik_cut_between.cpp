...
vector<u64> solve(const vector<vector<pii>>& g, const vector<u64>& terminals)
{
	...
	for (u64 S = 1; S < (1u << sz(terminals)); S++) {
		...
		for (u64 i = 0; i < n; i++) {
			if (is_terminal[i] and (S & is_terminal[i]))
...
int main()
{
	...
	for (u64 i = 0; i < v; i++) {
		...
		tmp.insert({ 0, u[i] - 1 });
		const auto k_v = sz(tmp), n = sz(gs[i]);
		const u64 first = !terminals[i].count(0);
		const u64 last = (1u << (k_v - 1)) * !terminals[i].count(u[i] - 1);
		const auto dp = solve(gs[i], vector<u64>(all(tmp)));
		const auto min_mask = [&](u64 mask) {
			return *min_element(&dp[n * mask], &dp[n * (mask + 1)]);
		};
		const u64 everyone = (1u << k_v) - 1;
		conn_both[i] = dp[n * everyone + 0];
		conn_r[i] = dp[n * (everyone & ~first) + u[i] - 1];
		conn_l[i] = dp[n * (everyone & ~last) + 0];
		if (sz(terminals[i]) == k)
			res = min(res, dp[n * (everyone & ~(first | last)) + *terminals[i].begin()]);
		conn_lr[i] = conn_both[i];
		for (u64 mask = (1u << k_v); mask-- > 0;) {
			if ((mask & 1u) != 0 or (mask & (1ull << (k_v - 1))) == 0)
				continue;
