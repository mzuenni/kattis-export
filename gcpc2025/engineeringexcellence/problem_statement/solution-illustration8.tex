    ...
    \begin{tikzpicture}[x=1cm,y=1cm,every node/.style={circle,inner sep=0pt,line width=1pt, minimum size = 0.66cm}]
        \begin{axis}[
            ...
            \begin{pgfonlayer}{foreground}
                \node[draw,fill=white] at (6cm,1cm) (a) {A};
                \node[draw,fill=white] at (6cm,2cm) (b) {B};
                \node[draw,fill=white] at (3cm,3cm) (c) {C};
                \node[draw,fill=white] at (1cm,2cm) (d) {D};
                \node[draw,fill=white] at (1cm,1cm) (e) {E};
            \end{pgfonlayer}
            \draw[fill=blue,opacity=0.6] (6cm,0cm) -- (b.center) -- (c.center) -- (d.center) -- (1cm,0cm) -- cycle;
            \draw[-,line width=0.01cm,line join=round,black] (a.center) -- (b.center) -- (c.center) -- (d.center) -- (e.center);

            \begin{scope}
                \clip (6cm,5cm) rectangle (d.center);
