   ...
   \begin{tikzpicture}[x=1cm,y=1cm,every node/.style={circle,inner sep=0pt,line width=1pt, minimum size = 0.66cm}]
        \begin{axis}[
            ...
            \begin{pgfonlayer}{foreground}
                \node[draw,fill=white] at (8cm,1cm) (a) {A};
                \node[draw,fill=white] at (8cm,2cm) (b) {B};
                \node[draw,fill=white] at (5cm,3cm) (c) {C};
                \node[draw,fill=white] at (3cm,2cm) (d) {D};
                \node[draw,fill=white] at (1cm,1cm) (e) {E};
                \node[draw,fill=white] at (5.5cm,4.5cm) (f) {};
            \end{pgfonlayer}
            \draw[fill=blue,opacity=0.6] (8cm,0cm) -- (b.center) -- (f.center) -- (d.center) -- (e.center) -- (1cm,0cm) -- cycle;
            ...
            \begin{scope}
                ...
                \draw[fill=red,opacity=0.5] (5.5cm,2cm) circle(2.5cm);
            \end{scope}

            \draw[->,line width=0.1cm,line join=round,black] (c.center) -- (f);
            \draw[red,line width=0.08cm] (4.68cm,3.15cm) -- (5.68cm,4.15cm);
            \draw[red,line width=0.08cm] (5.68cm,3.15cm) -- (4.68cm,4.15cm);
