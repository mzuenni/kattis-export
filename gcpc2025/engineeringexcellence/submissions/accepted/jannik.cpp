...
int main()
{
	...
	for (int i = 0; i < n; i++) {
		...
		const auto test = [&](pt p) {
			if (ccw(a - da, a, p) > 0 or ccw(c - dc, c, p) < 0)
				return;
			upd(p);
		};
		...
		if (opt = line_intersect(a, a + da, c, c + dc); abs(opt - center) < r + eps)
			...
			auto[p1,p2] = line_circle_intersect(a, da * pt(0, 1), center, r);
			test(p1), test(p2);
			tie(p1,p2) = line_circle_intersect(c, dc * pt(0, 1), center, r);
			test(p1), test(p2);
		}
	}
