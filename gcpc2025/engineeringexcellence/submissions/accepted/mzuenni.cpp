...
constexpr ld EPS = 1e-8;	//no idea if this works...
	...
	for (ll i = 0; i < n; i++) {
		...
		auto test = [&](pt p){
			if (dot(b-p, d-p) <= EPS		// angle at p >= 90° (alternative: norm(p-mid) <= rad2 + EPS)
				...
				//cout << fixed << setprecision(10) << c*0.5l << "->" << p*0.5l << ": " << 0.5*(abs(b-p) + abs(p-d) - abs(b-c) - abs(c-d)) << endl;
		...
		test(circleRayIntersection(mid, rad2, b, (b-a)));
		test(circleRayIntersection(mid, rad2, d, (d-e)));
	}
