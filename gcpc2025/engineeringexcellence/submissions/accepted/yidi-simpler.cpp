...
int main() {
    ...
    for (int i=2; i<n+2; i++) {
        ...
        if (abs(cross(lines[0].second - lines[0].first, lines[2].second - lines[2].first)) > EPS) {
            attempt(lineIntersection(lines[0].first, lines[0].second, lines[2].first, lines[2].second));
        }
        for (auto [x, y] : lines) {
