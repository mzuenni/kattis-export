...
#this solution does not consider, that there might be more than one door between two rooms for the same char
...
for _ in range(m):
    ...
    def append(s,t,ch):
        global graph 
        rooms=graph[s].get(ch, set())#SET is not the right data structure
        rooms.add(t)
        graph[s][ch]=rooms
    append(s,t,ch)
...
for ch in input():
    ...
    next_prob[-1]+=cur_prob[-1]#don't move if goal reached!
    cur_prob=next_prob
