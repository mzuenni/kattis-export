...
/*
 ...
 the program trys to change the order of the input, so it is always ascending modulo the value one trys
 to find so there no -1 class
 
 */

public class EgorDPSortSmartPicking {
	...
	private static int pickForMod(int[] gs){
		Arrays.sort(gs);
		int res=gs[0];//fall back
		
		out: for(int g:gs){
			for(int other: gs)
				if((other+1)%g==0)
					 continue out;//don't take this g, because there is one -1 mod g
			return g;
		}
		return res;
	}
