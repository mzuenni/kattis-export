...
public class MazePhilipp {
	public static void main(String[] args) {
		...
		ArrayList<HashMap<Character, List<Integer>>> edges = new ArrayList<>();
		for (int i = 0; i < n; i++) {
		...
		for (int i = 0; i < m; i++) {
			...
			if (!edges.get(a).containsKey(L)) {
				edges.get(a).put(L, new ArrayList<>());
			}
			edges.get(a).get(L).add(b);
			if (!edges.get(b).containsKey(L)) {
				edges.get(b).put(L, new ArrayList<>());
			}
		...
		for (char c : sequence.toCharArray()) {
			for (int i = 0; i < n; i++) {
				if (edges.get(i).get(c) == null || i == n-1) {
					nextProb[i] += prob[i];
					continue;
				}
