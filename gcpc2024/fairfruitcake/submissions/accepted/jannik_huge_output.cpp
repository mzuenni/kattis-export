...
void output(pt a, pt b, i64 d) {
	pt dir = a - b;
	if (dir.x == 0) dir.y = 1;
	else if (dir.y == 0) dir.x = 1;
	else {
		auto g = gcd(dir.x, dir.y);
		dir.x /= g, dir.y /= g;
	}
	const i64 MAXC = 1e9;
	i64 s = MAXC;
	if (dir.x != 0) s = min(s, dir.x > 0 ? (MAXC - a.x) / abs(dir.x) : (a.x - -MAXC) / abs(dir.x));
	if (dir.y != 0) s = min(s, dir.y > 0 ? (MAXC - a.y) / abs(dir.y) : (a.y - -MAXC) / abs(dir.y));
	pt pmax = a + pt(dir.x*s,dir.y*s);
	s = MAXC;
	if (dir.x != 0) s = min(s, dir.x > 0 ? (a.x - -MAXC) / abs(dir.x) : (MAXC - a.x) / abs(dir.x));
	if (dir.y != 0) s = min(s, dir.y > 0 ? (a.y - -MAXC) / abs(dir.y) : (MAXC - a.y) / abs(dir.y));
	pt pmin = a - pt(dir.x*s,dir.y*s);
	cout << frac(pmin.x, d) << ' ' << frac(pmin.y, d) << ' ' << frac(pmax.x, d) << ' ' << frac(pmax.y, d) << '\n';
	exit(0);
}
